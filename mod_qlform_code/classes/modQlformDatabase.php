...
class modQlformDatabase
{
	 ...
	 * @param   string    $table      Name of table to save data in
	 * @param   array     $data  data to be stored 
	 * @param   string    $primary    Name of primary key (data should include valid value for this)
	 *
	 * @return  mixed A database resource if successful, FALSE if not.
	 *
	 */
	public function saveupdate($table,$data,$primary)
	{
		$db=$this->getDatabase();
        $data=$this->objectToArrayOrTheOtherWay($data);
        return $db->updateObject($table,$data,$primary);
	}
